          ...
          return BlocBuilder<AssignmentReportCubit, AssignmentReportState>(
            builder: (context, state) {
              if (state is AssignmentReportFetchSuccess) {
                if ((state.submittedAssignments ?? 0) +
                        (state.unsubmittedAssignments ?? 0) ==
                    0) {
                  return NoDataContainer(titleKey: noAssignmentReportFoundKey);
                }
                  ...
                  child: Column(
                    children: [
                      totalAssignmentAndPointsSection(
                        ...
                        statsTitle1: UiUtils.getTranslatedLabel(
                          context,
                          submittedKey,
                        ),
                      ...
                      totalAssignmentAndPointsSection(
                        ...
                        progressValue:
                            int.parse(state.totalObtainedPoints!) /
                ...
                return SingleChildScrollView(
                  ...
                  child: Column(
                    children: [
                      ...
                      totalAssignmentAndPointsSection(
                        progressValue:
                            int.parse(state.totalObtainedMarks!) /
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: BlocBuilder<ReportTabSelectionCubit, ReportTabSelectionState>(
        builder: (context, state) {
          return Stack(
            children: [buildStudentReport(), buildReportAppBar(state)],
          );
        },
      ),
    );

...
class FeesDetailsScreenState extends State<FeesDetailsScreen> {
                ...
                Align(
                  ...
                  child: Padding(
                    padding: EdgeInsets.symmetric(
                      horizontal: 30 + UiUtils.screenContentHorizontalPadding,
                    ),
                    child: Text(
          ...
          Row(
            children: [
              ...
              if (feesData.isPaid && isCheckboxRequired)
                const Expanded(
                  child: Align(child: Icon(Icons.verified, color: greenColor)),
                ),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (feesData.isPaid &&
                          ...
                          Text(
                            '${UiUtils.getTranslatedLabel(context, paidOnKey)}: ${UiUtils.formatDate(feesData.paidDate ?? DateTime.now())}',
                            overflow: TextOverflow.ellipsis,
                            maxLines: 1,
                        ...
                        if ((isInstallment || isCompulsoryDueRow) &&
                            !feesData.isPaid)
                          Text(
                            '${UiUtils.getTranslatedLabel(context, dueDateKey)}: ${UiUtils.formatDate(feesData.dueDate ?? DateTime.now())}, ${UiUtils.getTranslatedLabel(context, chargesKey)}: ${feesData.dueChargesInPercentage}%',
                            overflow: TextOverflow.ellipsis,
                            maxLines: 2,
                            style: TextStyle(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (feesData.isDue &&
                        ...
                        Text(
                          UiUtils.formatAmount(
                            strVal: feesData.dueChargesAmount!.toStringAsFixed(
                              2,
                            ),
                            context: context,
                          ),
  ...
  void payNowProcess(ChildFees childFees) {
    ...
    //if only one option is on
      ...
      context.read<FeesPaymentCubit>().addFeesTransaction(
        ...
        paidDueCharges:
            dueChargesOnCompulsoryFees != 0.0 &&
                isCurrentlyBeingFullyPaid &&
                !isPayAsInstallment
            ? dueChargesOnCompulsoryFees
            : null,
        compulsoryAmountPaid: totalCompulsoryFees - dueChargesOnCompulsoryFees,
          ...
          if (state is FeesPaymentFetchSuccess) {
            ...
            if ((selectedPayment == 'paystack' ||
              ...
              final String paymentUrl = (selectedPayment == 'paystack')
                  ? state
                        .paymentGatewayDetails['payment_gateway_response']['data']['authorization_url']
            ...
            if (selectedPayment == 'stripe' &&
              ...
              clientSecret = state
                  .paymentGatewayDetails['payment_gateway_response']['client_secret'];
            }
            if (selectedPayment == 'paystack' &&
                context
                        .read<AppConfigurationCubit>()
  ...
  Widget _buildInstallmentsContainer({required ChildFees childFees}) {
    return Column(
      children: [
        ...
        if (isPayAsInstallment)
          listOfInstallments(fees: childFees.installmentData),
      ],
    );
  }
        ...
        children: [
          ...
          if (!thereAreNoFees)
            setRow(
              ...
              feesData: FeesData(id: 0, name: 'Total', amount: feesToBePaid),
  ...
  Widget _buildLoadingContainer() {
    return Align(child: _buildShimmerLoader());
  }
  ...
  Widget build(BuildContext context) {
    return BlocListener<StudentDetailedFeesCubit, StudentDetailedFeesState>(
      listener: (context, state) {
        ...
        */
        if (state is StudentDetailedFeesFetchSuccess) {
          if (!state.childFees.optionalFeesData.any(
            (element) => element.isPaid == false,
          )) {
            allOptionalPaid = true;
          }

          if (!state.childFees.compulsoryFeesData.any(
            (element) => element.isPaid == false,
          )) {
          ...
          if (state.childFees.compulsoryFeesDueDate != null &&
              ...
              !isCompulsoryFullyPaid) {
            if (state.childFees.compulsoryFeesDueDate!.isBefore(
                  state.childFees.currentDate,
                ) &&
                state.childFees.compulsoryFeesDueCharges! != 0) {
              isCompulsoryDue = true;
              dueChargesOnCompulsoryFees =
                  state.childFees.compulsoryFeesTotal *
          ...
          if (isInstallmentAvailable &&
            ...
            totalCompulsoryFees = state.childFees.compulsoryFeesData
                .fold<double>(
                  0,
                  (previousValue, element) =>
      ...
      child: PopScope(
        ...
        child: Scaffold(
          body: Stack(children: [detailedFeesContainer(), _buildAppBar()]),
        ),
      ),
    );
  }

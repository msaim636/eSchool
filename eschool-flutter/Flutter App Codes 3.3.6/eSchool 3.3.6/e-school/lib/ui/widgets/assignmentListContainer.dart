...
class AssignmentListContainer extends StatelessWidget {
  const AssignmentListContainer({
    required this.assignmentTabTitle, required this.currentSelectedSubjectId, required this.selectedAssignmentFilter, required this.isAssignmentSubmitted, super.key,
    this.childId,
  });
  ...
  List<Assignment> _getAssignmentsByAssignmentFilters(
    ...
    if (selectedAssignmentFilter == AssignmentFilters.dueDateLatest) {
      sortedAssignments.sort(
        (first, second) => second.dueDate.compareTo(first.dueDate),
      );
    } else if (selectedAssignmentFilter == AssignmentFilters.dueDateOldest) {
      sortedAssignments.sort(
        (first, second) => first.dueDate.compareTo(second.dueDate),
      );
              ...
              Navigator.of(context)
                  ...
                  .then((assignment) {
                    if (assignment != null) {
                      ...
                      context.read<StudentDashboardCubit>().updateAssignments(
                        assignment,
                      );

                      //update the same assignment if existing in the list in assignments cubit
                      context.read<AssignmentsCubit>().updateAssignments(
                        assignment,
                      );
                    }
                  });
